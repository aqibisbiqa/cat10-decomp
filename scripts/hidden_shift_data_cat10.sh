...
for qubit in $QUBIT_COUNTS; do
    for ccz in $CCZ_COUNTS; do
        for seed in $SEEDS; do
            ...
            # Cat10
            echo "hidden_shift_cat10_${qubit}_${ccz}_${seed}"
            if [ -f "hidden_shift_cat10_${qubit}_${ccz}_${seed}" ]; then
                echo "EXISTS $qubit $ccz $seed"
            else
                timeout $TIMEOUT ../../quizx/target/release/hidden_shift_stabrank_cat10 $qubit $ccz $seed
                if [ $? == 124 ]; then echo "TIMEOUT $qubit $ccz $seed"; fi
            fi
        done
    done
