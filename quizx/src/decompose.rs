...
impl<'a, G: GraphLike> Decomposer<G> {
    ...
    pub fn use_more_cats(&mut self, b: bool) -> &mut Self {
        self.use_more_cats = b;
        self
    }
    ...
    fn push_cat_decomp(&mut self, depth: usize, g: &G, verts: &[V]) -> &mut Self {
        ...
        }else if verts[1..].len() == 10 {
            self.push_decomp(&[
                Decomposer::replace_cat10_00,
                Decomposer::replace_cat10_01,
                Decomposer::replace_cat10_02,
                Decomposer::replace_cat10_10,
                Decomposer::replace_cat10_11,
                Decomposer::replace_cat10_12,
                Decomposer::replace_cat10_20,
                Decomposer::replace_cat10_21,
                Decomposer::replace_cat10_22,
            ], depth, &g, &verts)
    ...
    fn replace_cat10_00(g: &G, verts: &[V]) -> G {
        let mut g = g.clone();
        *g.scalar_mut() *= ScalarN::Exact(-2, vec![1, 0, 0, 0]);    
        for &v in &verts[1..] {
            g.add_to_phase(v, Rational::new(-1,4));
            g.set_edge_type(v, verts[0], EType::N);
        }
        g.set_phase(verts[0], Rational::new(3,2));
        g
    }

    fn replace_cat10_01(g: &G, verts: &[V]) -> G {
        let mut g = g.clone();
        *g.scalar_mut() *= ScalarN::Exact(-2, vec![-1, 0, 1, 0]);    
        let b = g.add_vertex_with_phase(VType::Z, Rational::new(1,1));
        for &v in &verts[1..5] {
            g.remove_edge(v, verts[0]);
            g.add_edge_with_type(v, b, EType::N);
            g.add_to_phase(v, Rational::new(-1,4));
        }
        g.add_edge_with_type(verts[0], b, EType::H);
        for &v in &verts[6..10] {
            g.add_to_phase(v, Rational::new(-1,4));
        }
        g
    ...
    fn replace_cat10_02(g: &G, verts: &[V]) -> G {
        let mut g = g.clone();
        *g.scalar_mut() *= ScalarN::Exact(-2, vec![-1, 0, -1, 0]);
        let b = g.add_vertex_with_phase(VType::Z, Rational::new(3,2));
        for &v in &verts[1..5] {
            g.remove_edge(v, verts[0]);
            g.add_edge_with_type(v, b, EType::N);
            g.add_to_phase(v, Rational::new(-1,4));
    ...
    fn replace_cat10_10(g: &G, verts: &[V]) -> G {
        Decomposer::replace_cat10_01(g, &vec![
                                    verts[0],
                                    verts[10],
                                    verts[9],
                                    verts[8],
    ...
    fn replace_cat10_11(g: &G, verts: &[V]) -> G {
        let mut g = g.clone();
        *g.scalar_mut() *= ScalarN::Exact(-1, vec![0, 0, -1, 0]);
        ...
        let p = g.add_vertex_with_phase(VType::Z, Rational::new(1,2));
        g.add_edge_with_type(b, p, EType::H);
        g.add_edge_with_type(verts[0], p, EType::H);
        for &v in &verts[6..10] {
            g.add_to_phase(v, Rational::new(-1,4));
        }
        g
    ...
    fn replace_cat10_12(g: &G, verts: &[V]) -> G {
        let mut g = g.clone();
        *g.scalar_mut() *= ScalarN::Exact(-1, vec![1, 0, 0, 0]);
        let b = g.add_vertex(VType::Z);
        for &v in &verts[1..5] {
        ...
        let p = g.add_vertex_with_phase(VType::Z, Rational::new(1,1));
        g.add_edge_with_type(b, p, EType::H);
        g.add_edge_with_type(verts[0], p, EType::H);
        for &v in &verts[6..10] {
            g.add_to_phase(v, Rational::new(1,4));
    ...
    fn replace_cat10_20(g: &G, verts: &[V]) -> G {
        Decomposer::replace_cat10_02(g, &vec![
                                    verts[0],
                                    verts[10],
                                    verts[9],
                                    verts[8],
    ...
    fn replace_cat10_21(g: &G, verts: &[V]) -> G {
        Decomposer::replace_cat10_12(g, &vec![
                                    verts[0],
                                    verts[10],
                                    verts[9],
                                    verts[8],
    ...
    fn replace_cat10_22(g: &G, verts: &[V]) -> G {
        let mut g = g.clone();
        *g.scalar_mut() *= ScalarN::Exact(-1, vec![0, 0, 1, 0]);
        let b = g.add_vertex(VType::Z);
        for &v in &verts[1..5] {
        ...
        let p = g.add_vertex_with_phase(VType::Z, Rational::new(3,2));
        g.add_edge_with_type(b, p, EType::H);
        g.add_edge_with_type(verts[0], p, EType::H);
        for &v in &verts[6..10] {
            g.add_to_phase(v, Rational::new(1,4));
